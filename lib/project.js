...
const getIncludeFiles = function (patterns, templatePath) {
  ...
  return glob.sync('**/?('+ patterns.join('|') +')', {
    cwd: path.join(templatePath, TEMPLATE_DIR_NAME)
  })
}
    ...
    options(projectName, templatePath)
      .then((metas) => {
        ...
        async.each(includeFiles, (file, next) => {
          let filePath = path.join(templatePath, TEMPLATE_DIR_NAME, file)
          ...
          fs.copySync(path.join(templatePath, TEMPLATE_DIR_NAME), projectPath)
          resolve('')
